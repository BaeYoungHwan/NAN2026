...
- [x] 스테이지 지형(벽/통로)·광원·스폰·골 위치 절차적 생성 (Procedural Generation — 시드 기반 carve, 재현 가능)
- [x] 벽-캐릭터 충돌판정 (축 분리 이동, 벽 슬라이딩)
- [x] 골 도달 시 클리어 판정
- [ ] 플레이어 이동 패턴 학습 → 자주 가는 동선 차단 AI 알고리즘
- [x] 라운드 진행 시스템 — 1R(안전 구역 가이드라인 표시) → 2R(가이드라인 제거) → 3R 진행 로직 (`src/core/round.ts`, `wonho` 브랜치 병합 완료)
