...
NHN 주최 NAN2026 AI 게임 개발 해커톤 사전과제용 웹 게임. 광원(태양)은 스테이지마다 고정된 위치를 가지며, 캐릭터 위치에 따라 그림자의 궤적/각도가 매 프레임 실시간으로 계산된다. 플레이어는 캐릭터를 이동시켜 이 그림자를 간접적으로 조종하는 키보드 전용 액션 퍼즐 싱글 플레이 게임으로, 그림자가 스테이지에 정의된 안전 경계(바운더리)를 벗어나는 순간 즉사·스테이지 리셋된다. 스테이지 지형·광원 위치·안전 경계는 절차적으로 생성되고, 플레이어의 이동 패턴을 학습해 자주 다니는 동선을 차단하는 AI 알고리즘이 핵심 차별점.
...
- 그림자 경계 이탈 판정·각도 계산 로직은 항상 예외 상황(광원-캐릭터 동일 좌표, 화면 경계 등)과 판정 기준을 명확히 제시한다.
