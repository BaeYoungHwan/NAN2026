...
### 프로젝트: NAN2026 — 돌려돌려 그림자

NHN 주최 NAN2026 AI 게임 개발 해커톤 사전과제용 웹 게임. 플레이어는 캐릭터(WASD 이동)와 그림자(`,`/`.` 회전)를 동시에 조종하는 이중 조작 키보드 전용 액션 퍼즐 싱글 플레이 게임이다. 광원(태양)은 스테이지마다 고정된 위치를 가지며, 캐릭터 뒤(광원 반대 방향) ± 허용 각도가 "안전 구역"으로 매 프레임 재계산된다. 그림자는 이 안전 구역과 무관하게 플레이어가 직접 회전시킨 각도를 그대로 유지하므로, 캐릭터를 움직여 안전 구역이 이동할 때마다 그림자도 맞춰 회전시켜야 한다 — 그림자 각도가 안전 구역을 벗어나는 순간 즉사·스테이지 리셋된다. 스테이지 지형·광원 위치는 절차적으로 생성되며, 1R~3R은 각각 독립적으로 생성된 별도 스테이지다(결정론적으로 파생된 시드) — 라운드가 올라갈 때마다 완전히 새로운 지형으로 전환된다.

- 사전과제 제출 마감: 2026-08-10
- 참가팀 발표: 2026-08-22
- 본행사(48시간 개발, 주제 현장 공개): 2026-09-04 ~ 09-06
- 팀: 배영환(백엔드/아키텍처/WBS 총괄) + 송원호(프론트엔드·UI/콘텐츠/QA)
- 저장소: https://github.com/BaeYoungHwan/NAN2026

### Claude 행동 지침

- 사전과제 제출물 5종(플레이 가능 빌드, 플레이 영상, 게임소개 PDF, AI활용기술문서 PDF, 팀원 롤 기술서 PDF) 중 누락되는 항목이 없도록 항상 진행 상황을 인지한다.
- 제출물 문서(게임소개·AI활용기술문서·팀원 롤 기술서) 원고는 `docs/product-specs/`가 아니라 `docs/submission/`에 둔다 — 기획/작업용 문서와 제출용 최종본을 디렉터리로 분리(PR #23 리뷰 반영).
...
- 그림자 각도 판정 로직은 항상 예외 상황(광원-캐릭터 동일 좌표, 각도 wrap-around, 화면 경계 등)과 판정 기준을 명확히 제시한다.
- 답변은 항상 존댓말로 짧고 간결하게 작성하고, 불확실한 내용은 추측하지 않고 모른다고 말한다. 불필요한 미사여구·아부성 표현은 사용하지 않는다.
...
- 사용자 회원가입 및 로그인
- 랭킹 시스템 및 글로벌 리더보드 (서버 기반 기능)
- 마우스 컨트롤 지원 (키보드 전용 유지)
- 아이템, 스킬, 광원 직접 조작 기믹
- 복잡한 그래픽 셰이더 및 3D 그래픽 효과
- PC 실행파일(.exe) 빌드
- 유료 라이선스가 필요한 에셋/에셋 스토어 항목
- 4R 이상 라운드 — 이번 사전과제 데모는 1R(가이드라인 표시)~3R(디메리트)까지만 구현, 4R은 본선/이후 개발로 보류

### 기술 스택 고정

> 팀 최종 확정 전까지 임의로 다른 라이브러리·프레임워크를 도입하지 않는다.

- TypeScript + React + HTML5 Canvas (확정 — UI는 React, 게임 렌더링/물리는 Canvas 직접 제어)
- 절차적 스테이지 생성은 클라이언트(JS) 측에서 수행 — 서버 비용 최소화
- 확정 조건: 웹 브라우저 실행 필수, 키보드 단독 조작만으로 완전 플레이 가능해야 함
- 배포: GitHub Pages (필수)
