...
- **인코딩**: 128 kbps MP3(libmp3lame, `-write_xing 0`). Xing 헤더의 인코더 지연 정보가 일부 디코더에서 앞뒤 무음으로 해석돼 루프에 틈을 만들 수 있어 뺐다. 전 곡 합계 **24MB → 5.5MB**.
- 가공에는 ffmpeg(디코딩·인코딩)와 1회성 Node 스크립트(트림·크로스페이드·정규화)를 썼다. 원본 파일은 저장소에 남기지 않는다 — 위 URL에서 언제든 다시 받을 수 있다.
