...
const REGIONS = [
  ...
  // PR #17 리뷰: 다리 사이 erase(x=584~610)가 왼발 끝(585)·오른발 시작(608)을
  // 1~3px 침범하고 있었다. 신발 바깥쪽 erase(왼쪽 x=548~571)가 왼발 시작(572)
  // 바로 앞에서 멈춰 발을 안 건드리는 것과 같은 "실측 경계에서 1px 띄운다"
  // 규칙을 다리 사이·오른쪽 바깥쪽에도 동일하게 맞춰 재조정.
  { name: "body-idle-pose", x: 548, y: 432, w: 105, h: 181, erase: [
    { x: 610, y: 460, w: 25, h: 20 },
    { x: 586, y: 565, w: 22, h: 47 },
    { x: 548, y: 601, w: 24, h: 11 },
    { x: 624, y: 601, w: 14, h: 11 },
  ] },
  ...
  // 지웠던 것과 동일 위치)도 함께 지운다. 속도선 erase 2줄은 body-run-legs와
  // 같은 절대 시트 좌표를 공유하는 영역이라(둘 다 x=760~903 원본에서 추출),
  // 거기서 실측해 좁힌 범위(다리를 가로지르지 않는 y581~587,x776~850 /
  // y592~597,x805~856)를 여기도 그대로 맞춘다 — 이 파일은 실제 게임에서
  // 로드되지 않는 참고용 크롭이지만 좌표가 어긋난 채로 남아있으면 헷갈린다.
