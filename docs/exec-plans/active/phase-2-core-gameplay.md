...
- [x] 플레이 영상 (2026-08-07 유튜브 업로드 — <https://www.youtube.com/watch?v=zsrd9mrBQdI>. 링크만 제출, 원본 파일은 저장소에 넣지 않음 — [[project-submission-video-hosting]]). 전체 공개 + 시크릿 창 재생 확인 완료
- [x] 게임소개 PDF — 스크린샷 5컷 삽입·플레이 영상 링크 삽입 완료, 2026-08-07 재변환(1171 KB)
- [x] AI활용기술문서 PDF — 양쪽 파트 작성 완료(PR #22), PDF 변환 완료. BGM 5곡 라이선스는 곡별로 검증해 `public/assets/ASSET_SOURCES.md`·`src/content/credits.ts`에 기록·이행 완료(CC0 3곡, CC-BY 2곡). **생성형 AI 이미지 약관도 2026-08-07 확인 완료** — 자동화 도구는 403으로 막혀 실제 브라우저로 [OpenAI 이용약관](https://openai.com/policies/terms-of-use/)(발효일 2026-01-01) 원문의 "콘텐츠의 소유권" 조항을 직접 읽었다. 출력의 모든 권리가 사용자에게 양도되고 상업적 이용 제한 문구가 없어 제약 없음. 부수 확인 2건: 소유하되 독점은 불가("콘텐츠 유사성"), AI 생성 사실을 감추면 약관 위반이므로 출처 기록이 곧 약관 준수 수단이다. 인용은 `ASSET_SOURCES.md` "생성형 AI 이미지 라이선스 확인" 절
  - 대회 요강·FAQ상 AI 생성 에셋 규정은 **명문 규정이 없어 팀 판단으로 진행**하기로 결정(2026-08-07) — AI 게임 개발 해커톤이고 제출물에 "AI 활용 기술 문서"가 포함된 이상 AI 활용은 금지가 아니라 평가 대상으로 본다. 에셋 출처·라이선스를 전량 기록해 뒀으므로 확인 요청 시 즉시 제출 가능하다는 것이 대비책이다
...
- [x] **시크릿 창(로그아웃 상태)에서 재생 확인 (2026-08-07)** — 전체 공개 상태로 정상 재생 확인. 자동 조회로는 검증되지 않는 항목이었다(`WebFetch`로는 페이지 제목만 나오고 공개 범위는 YouTube가 JS로 렌더한다)
