...
| 그림자 경계 판정 방식 | 기하학적/벡터 방식 (point-in-polygon + 선분-다각형 교차) | 그리드는 셀 양자화 오차로 "억울한 죽음" 리스크 증가, 픽셀은 매 프레임 Canvas readback으로 60 FPS·저사양 환경 요건과 충돌. 기하 방식은 해석적으로 정확하고 readback 없이 O(n) 연산으로 성능 여유가 큼 | - |
