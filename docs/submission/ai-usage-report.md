...
**크롭 버그** — 배경 장식(잉크 방울·바닥 그림자·속도선) 제거용 erase 사각형이 머리·발·다리 실루엣까지 지워버리는 문제를, 실제 스프라이트 렌더링 스크린샷으로 발견해 좌표를 재보정했다.
...
공개된 대회 안내·보도자료·FAQ에 AI 생성 에셋에 관한 언급 자체가 없다. 다만 이번 과제가 **AI 게임 개발 해커톤**이고 제출물 5종에 "AI 활용 기술 문서"가 포함돼 있다는 점에서, AI 활용은 금지 대상이 아니라 **평가 대상**이라고 판단해 진행했다.
